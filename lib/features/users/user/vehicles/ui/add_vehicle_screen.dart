...
import '../logic/add_vehicle/add_vehicle_cubit.dart';
import '../logic/add_vehicle/add_vehicle_state.dart';
import 'widgets/vehicle_form_fields.dart';

/// شاشة إضافة سيارة جديدة.
class AddVehicleScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => AddVehicleCubit(getIt()),
      child: const _AddVehicleView(),
    );
  }
}

class _AddVehicleView extends StatelessWidget {
      ...
      body: BlocConsumer<AddVehicleCubit, AddVehicleState>(
        listener: (context, state) {
          state.whenOrNull(
            success: (_) {
              showFlashMessage(
                message: LocaleKeys.vehicleAddedMsg.tr(),
